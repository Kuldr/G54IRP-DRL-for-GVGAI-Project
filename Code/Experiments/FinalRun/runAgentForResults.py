...
envs = [ env.id for env in gym.envs.registry.all() if env.id.startswith('gvgai-aliens') or env.id.startswith('gvgai-boulderdash') or env.id.startswith('gvgai-missilecommand') ]
random.shuffle(envs)
...
# Save as a json file
with open('experimentFinalResults.json') as data_file:
    results = json.load(data_file)
    for r in results:
        gameResults.append(r)
with open("experimentFinalResults.json", "w") as outfile:
