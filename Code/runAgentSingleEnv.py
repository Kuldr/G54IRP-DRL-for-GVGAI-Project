...
envs = [env.id for env in gym.envs.registry.all() if env.id.startswith('gvgai-missilecommand') or env.id.startswith('gvgai-boulderdash')]
...
agent = Agent.Agent()
infEnd = datetime.now()
print("Initialise Time= " + str((infEnd-infStart).total_seconds()*1000))
